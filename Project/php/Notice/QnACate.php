                            ...
                            if($count > 0){
                                for($i = 1; $i <= $count; $i++) {
                                    ...
                                    echo "<div class='icon'>
                                            ...
                                            <a href='QnAAnswer.php?QnAID=$QnAID&QnACategory=$category&myMemberID=$QnAMyMemberID' class='answer'>내가알려줄게</a>
                                        </div>";
                                    echo "<div class='QnA__date'>".date('Y-m-d', $QnAregTime)."</div>";

                                    $myMemberID = $_SESSION['myMemberID'];
                                    $second_sql = "SELECT r.youName,r.myReplyID,r.QnAReply, r.great, r.regTime,r.myQnAID FROM myReply r JOIN myQnA q ON (r.myQnAID = q.myQnAID) WHERE r.myQnAID=$QnAID and r.great='no' ORDER BY r.regTime DESC;";
                                    $second_result = $connect -> query($second_sql);

                                    echo "<div class='cont__wrap'>";
                                    echo "<div class='QnA__cont contents'>";
                                    echo "<div class='contents__title'>내용</div>";
                                    if($QnAMyMemberID === $_SESSION['myMemberID']) {
                                        echo "<a href='QnAModify.php?QnAID=$QnAID'>수정해야지</a>";
                                    }
