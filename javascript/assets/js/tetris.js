...
const blocks = {
  Tmino: [
    [[2,1],[0,1],[1,0],[1,1]],  // Tmino의 기본 모양
    [[2,1],[1,2],[1,0],[1,1]],  // Tmino의 돌아간 모양2
    [[1,2],[0,1],[2,1],[1,1]],  // Tmino의 돌아간 모양3
    [[1,2],[0,1],[1,0],[1,1]],  // Tmino의 돌아간 모양4
  ],
  Imino : [
      [[0,0],[0,1],[0,2],[0,3]],
      [[0,0],[1,0],[2,0],[3,0]],
...
function renderBlocks(moveType = ""){
  if(!stopTetris){
    ...
    blocks[type][direction].some(block => {
      ...
      if(isAvailabe){ // 테두리에 도달하지 않음. 정상영역
        target.classList.add(type, "moving")  // 클래스를 추가해 블록이 보이도록 해줍니다.
      } else {  //테두리에 도달한 경우 위치 값에 이동하는 값을 새로 넣지 않고 원래의 값을 넣어줍니다.
        tempMovingItem = { ...movingItem };
...
function tetrisGameover(){
  ...
  document.querySelector(".tetris__restart").classList.add("show");
}
